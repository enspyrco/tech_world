...
This is a **deployment-shape contract**, surfaced to the engine as a thin Dart config value. The engine holds a config (URL + auth strategy); the real implementation is whatever HTTP service stands at that URL. Reference implementations live in `examples/livekit-token-server/` (the Dart credential-JWT reference). The realized production server is **`enspyrco/realm-token-server`** (Node, deployed on OCI) — `POST /exchange` (verify Firebase ID token → mint Realm credential) + `POST /livekit-token` (verify credential → mint LiveKit token), same ES256 credential contract as the Dart reference.
