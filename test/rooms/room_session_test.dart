...
void main() {
  ...
  group('presence lifecycle', () {
    ...
    test('presence is re-stamped on a heartbeat while the session lives',
        () async {
      // The foyer drops entries older than PresenceService.presenceTtl, so a
      // connected player who stands still must keep refreshing or they vanish
      // from it. The TTL and this heartbeat are one mechanism — a TTL without a
      // heartbeat trades a ghost for a disappearing player, which is worse.
      final liveKit = _FakeLiveKit();
      _stubLiveKit(liveKit);
      when(liveKit.connect)
          .thenAnswer((_) async => ConnectionResult.connected);
      ...
      await session.connect();
      expect(spy.enteredRooms.length, 1, reason: 'the connect enter');

      await Future<void>.delayed(const Duration(milliseconds: 70));
      expect(spy.enteredRooms.length, greaterThan(1),
          reason: 'lastSeen must keep being refreshed while connected');

      await session.leave();
    });

    test('the heartbeat stops on leave and cannot resurrect the ghost',
        () async {
      // A beat landing after leave()'s delete would rewrite the very document
      // the delete removed — the exact resurrection the enter/leave
      // serialization already guards against for connect->leave.
      final liveKit = _FakeLiveKit();
      _stubLiveKit(liveKit);
      when(liveKit.connect)
          .thenAnswer((_) async => ConnectionResult.connected);
      ...
      final afterLeave = spy.order.length;
      await Future<void>.delayed(const Duration(milliseconds: 80));

      expect(spy.order.length, afterLeave,
          reason: 'no presence operation may occur after leave');
      expect(spy.order.last, startsWith('leave:'),
          reason: 'the delete must remain the last writer');
    });
