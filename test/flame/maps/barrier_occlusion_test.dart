...
void main() {
  ...
  group('buildObjectLayerFromBarriers with WallDef', () {
    // A minimal WallDef for testing — all bitmasks map to known tile indices.
    final testWallDef = WallDef(
      id: 'test_wall',
      name: 'Test Wall',
      tilesetId: 'construction',
      faceBitmaskToTileIndex: {
        for (var i = 0; i < 16; i++) i: 100 + i,
      },
      capBitmaskToTileIndex: {
        for (var i = 0; i < 16; i++) i: 200 + i,
      },
    );

    test('places construction face tiles at barrier positions', () {
      final floor = TileLayerData();
      floor.setTile(5, 7, const TileRef(tilesetId: 'test', tileIndex: 99));
      ...
      // Should use construction tile, not copied floor tile.
      final ref = objectLayer.tileAt(5, 7)!;
      expect(ref.tilesetId, equals('construction'));
      // Isolated barrier: bitmask 0 → face tile 100.
      expect(ref.tileIndex, equals(100));
    });

    test('places wall cap tiles at y-1 above north-facing barriers', () {
      final floor = TileLayerData();
      final barriers = {(5, 7), (6, 7), (7, 7)}; // horizontal wall
      ...
      // All are north-facing → caps at y=6.
      expect(objectLayer.tileAt(5, 6), isNotNull);
      expect(objectLayer.tileAt(6, 6), isNotNull);
      expect(objectLayer.tileAt(7, 6), isNotNull);

      // Cap tiles come from construction tileset.
      expect(objectLayer.tileAt(5, 6)!.tilesetId, equals('construction'));
      expect(objectLayer.tileAt(6, 6)!.tilesetId, equals('construction'));
    });

    test('does not place caps above non-north-facing barriers', () {
      final floor = TileLayerData();
      // Vertical wall: only (5,7) is north-facing.
      final barriers = {(5, 7), (5, 8), (5, 9)};
      ...
      // Only (5,6) gets a cap — (5,7) and (5,8) are not north-facing.
      expect(objectLayer.tileAt(5, 6), isNotNull);
      expect(objectLayer.tileAt(5, 7), isNotNull); // face tile
      // (5,7) position is NOT a cap — it's a face for barrier (5,8).
    });

    test('excludes vertical doorway lintel from caps', () {
      final floor = TileLayerData();
      floor.setTile(4, 15, const TileRef(tilesetId: 'test', tileIndex: 1));

      // Vertical doorway: gap at y=17, wall resumes at y=18.
      final barriers = {(4, 15), (4, 16), (4, 18)};
      ...
      // (4,15) should be a floor-copied extended occlusion tile, NOT a
      // construction cap — because (4,16) is a vertical doorway lintel.
      final ref = objectLayer.tileAt(4, 15)!;
      expect(ref.tilesetId, equals('test'),
          reason: 'vertical doorway lintel extended occlusion should copy '
              'from floor, not use construction cap');
    });

    test('horizontal doorway lintel tiles copy from floor layer', () {
      final floor = TileLayerData();
      floor.setTile(7, 22, const TileRef(tilesetId: 'test', tileIndex: 70));
      ...
      // Gap lintel tile above (7,22) should still be floor-copied.
      final ref = objectLayer.tileAt(7, 22)!;
      expect(ref.tilesetId, equals('test'));
      expect(ref.tileIndex, equals(70));
    });

    test('face bitmask reflects cardinal neighbors', () {
      final floor = TileLayerData();
      // Horizontal wall: middle barrier has E+W neighbors.
      final barriers = {(5, 7), (6, 7), (7, 7)};
      ...
      // Middle barrier (6,7): E+W = 2+8 = 10 → face tile 110.
      expect(objectLayer.tileAt(6, 7)!.tileIndex, equals(110));
      // Left end (5,7): E only = 2 → face tile 102.
      expect(objectLayer.tileAt(5, 7)!.tileIndex, equals(102));
      // Right end (7,7): W only = 8 → face tile 108.
      expect(objectLayer.tileAt(7, 7)!.tileIndex, equals(108));
    });
  });
