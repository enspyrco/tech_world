...
**Dreamfinder's hearing is territory-gated, but `proximityRadius` still owns the kill switch.** The signal is a CONJUNCTION: `proximityRadius > 0 && territory.contains(playerGrid)`. Territory *narrows* who he hears; it does not *replace* the preference that governs every other proximity gate. Radius 0 therefore silences him too — without the `> 0` term a player who set the preference to zero was still heard while standing on his square, with no bubble and no audio to reveal it (found by Tesla, cage-match PR #530). Only the zero-ness is consulted: comparing the radius to a *distance* here is the coupling below that PR #529 removed. The `df-proximity` signal publishes territory *containment* — he hears only players standing inside his authored square (`TerritoryRect.contains`, the same rect the overlay draws). It previously measured Chebyshev distance to his sprite, and since he wanders *within* his square that let him hear players standing outside the box beside him. This deliberately breaks the PR #481 coupling that tied the hearing thresholds to the audio gate's pair: he hears you only inside the box, while you still hear him by proximity from outside it.
